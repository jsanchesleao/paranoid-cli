...
async function lockFile({file, password, output}) {
  ...
  const destination = output ? output : fs.createWriteStream(`${file}.lock`);
...
async function unlockFile({file, password, output}) {
  ...
  const destination = output ? output : fs.createWriteStream(file.replace(/\.lock$/, ''));
